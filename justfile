...
# Import map: rewrites canonical URI imports (e.g. `dpv:schema/dpv_common`) to
...
# otherwise produce a doubled `src/dpv/schema/src/dpv/...` path).
...
clean: _wsl2_compat_check _clean_project
  rm -rf tmp
  rm -rf {{docdir}}/*.md
  # remove generated dpvcg test fixtures (regenerated by `_gen-fixtures`),
  rm -rf tests/data/dpvcg/valid/* tests/data/dpvcg/invalid/* tests/data/dpvcg/problem/*
...
# `dpv:schema/dpv_common` to be fetched over HTTP (404 against
...
# (`tmp/dpv.yaml`), which has no `imports:` left to mis-resolve.
[group('model development')]
gen-doc: _merged-schema gen-doc-extensions && _add-artifacts
...
#
# Use the pre-merged, self-contained schema (`tmp/dpv.yaml`) for the same
...
_clean_project:
    ...
    # from upstream-releases/dpv/). The top-level `dpv.yaml`, the
    # 8 semantic-group `dpv_<group>.yaml` schemas, every per-module schema under `modules/`,
    # and every per-extension schema under `extensions/` (including nested
    # subtrees such as `extensions/legal/eu/...`) are all derived artifacts;
    # the mappings overlay (`apply-sssom-overlay`) is then re-applied on top
    # during the build. We walk the entire `schema/` tree recursively so
    # newly added extension subdirectories are cleaned without needing a
    # justfile update.
    ...
    if schema_dir.is_dir():
        for f in sorted(schema_dir.rglob("*.yaml")):
            print(f'removing "{f}"')
            f.unlink()
        # Prune any directories left empty by the yaml sweep (deepest first),
        # but keep `schema_dir` itself.
        for d in sorted(
            (p for p in schema_dir.rglob("*") if p.is_dir()),
