...
Schema-agnostic: pass any LinkML schema YAML with ``--schema``.

Usage
-----
  uv run python scripts/debug_each_generator.py --schema path/to/schema.yaml
  uv run python scripts/debug_each_generator.py --schema path/to/schema.yaml \
...
def _run_generator_child(
    ...
    try:
        with open(stack_path, "w", encoding="utf-8") as sf:
            ...
            if verbose:
                print(f"[debug-each-child] starting {gen_name}", flush=True)
...
def main():
    ...
    parser.add_argument(
        "--schema", required=True, type=Path, help="Path to LinkML schema YAML"
    )
    parser.add_argument(
        "--outdir",
        type=Path,
        default=None,
        help="Directory for per-generator output files "
             "(default: /tmp/<schema-stem>-gen-each)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if args.outdir is None:
        args.outdir = Path(f"/tmp/{schema_path.stem}-gen-each")
    if args.stack_log is None:
        args.stack_log = str(args.outdir / "stacks")
