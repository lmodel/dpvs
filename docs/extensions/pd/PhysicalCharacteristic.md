---
search:
  boost: 10.0
---

# Class: PhysicalCharacteristic 


_Information about physical characteristics_



<div data-search-exclude markdown="1">



URI: [pd:PhysicalCharacteristic](https://w3id.org/lmodel/dpv/pd/PhysicalCharacteristic)





```mermaid
 classDiagram
    class PhysicalCharacteristic
    click PhysicalCharacteristic href "../PhysicalCharacteristic/"
      External <|-- PhysicalCharacteristic
        click External href "../External/"
      

      PhysicalCharacteristic <|-- Age
        click Age href "../Age/"
      PhysicalCharacteristic <|-- Gender
        click Gender href "../Gender/"
      PhysicalCharacteristic <|-- HairColour
        click HairColour href "../HairColour/"
      PhysicalCharacteristic <|-- Height
        click Height href "../Height/"
      PhysicalCharacteristic <|-- Piercing
        click Piercing href "../Piercing/"
      PhysicalCharacteristic <|-- SkinTone
        click SkinTone href "../SkinTone/"
      PhysicalCharacteristic <|-- Tattoo
        click Tattoo href "../Tattoo/"
      PhysicalCharacteristic <|-- Weight
        click Weight href "../Weight/"
      

      
```





## Inheritance
* [External](External.md)
    * **PhysicalCharacteristic**
        * [Age](Age.md)
        * [Gender](Gender.md)
        * [HairColour](HairColour.md)
        * [Height](Height.md)
        * [Piercing](Piercing.md)
        * [SkinTone](SkinTone.md)
        * [Tattoo](Tattoo.md)
        * [Weight](Weight.md)


## Class Properties

| Property | Value |
| --- | --- |
| Class URI | [pd:PhysicalCharacteristic](https://w3id.org/lmodel/dpv/pd/PhysicalCharacteristic) |


## Slots

| Name | Cardinality and Range | Description | Inheritance |
| ---  | --- | --- | --- |











## In Subsets


* [PdSubset](PdSubset.md)



## Aliases


* Physical Characteristic




## Identifier and Mapping Information



### Annotations

| property | value |
| --- | --- |
| upstream_iri | https://w3id.org/dpv/pd/owl#PhysicalCharacteristic |
| dpv_extension_slug | pd |




### Schema Source


* from schema: https://w3id.org/lmodel/dpv/pd




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | pd:PhysicalCharacteristic |
| native | pd:PhysicalCharacteristic |
| exact | dpv_pd:PhysicalCharacteristic, dpv_pd_owl:PhysicalCharacteristic |
| related | svd:Demographic |






## LinkML Source

<!-- TODO: investigate https://stackoverflow.com/questions/37606292/how-to-create-tabbed-code-blocks-in-mkdocs-or-sphinx -->

### Direct

<details>
```yaml
name: PhysicalCharacteristic
annotations:
  upstream_iri:
    tag: upstream_iri
    value: https://w3id.org/dpv/pd/owl#PhysicalCharacteristic
  dpv_extension_slug:
    tag: dpv_extension_slug
    value: pd
description: Information about physical characteristics
in_subset:
- pd_subset
from_schema: https://w3id.org/lmodel/dpv/pd
aliases:
- Physical Characteristic
exact_mappings:
- dpv_pd:PhysicalCharacteristic
- dpv_pd_owl:PhysicalCharacteristic
related_mappings:
- svd:Demographic
is_a: External
class_uri: pd:PhysicalCharacteristic

```
</details>

### Induced

<details>
```yaml
name: PhysicalCharacteristic
annotations:
  upstream_iri:
    tag: upstream_iri
    value: https://w3id.org/dpv/pd/owl#PhysicalCharacteristic
  dpv_extension_slug:
    tag: dpv_extension_slug
    value: pd
description: Information about physical characteristics
in_subset:
- pd_subset
from_schema: https://w3id.org/lmodel/dpv/pd
aliases:
- Physical Characteristic
exact_mappings:
- dpv_pd:PhysicalCharacteristic
- dpv_pd_owl:PhysicalCharacteristic
related_mappings:
- svd:Demographic
is_a: External
class_uri: pd:PhysicalCharacteristic

```
</details></div>