---
search:
  boost: 10.0
---

# Class: Identifying 


_Information that uniquely or semi-uniquely identifies an individual or a_

_group_



<div data-search-exclude markdown="1">



URI: [pd:Identifying](https://w3id.org/lmodel/dpv/pd/Identifying)





```mermaid
 classDiagram
    class Identifying
    click Identifying href "../Identifying/"
      External <|-- Identifying
        click External href "../External/"
      

      Identifying <|-- Biometric
        click Biometric href "../Biometric/"
      Identifying <|-- Name
        click Name href "../Name/"
      Identifying <|-- OfficialID
        click OfficialID href "../OfficialID/"
      Identifying <|-- Picture
        click Picture href "../Picture/"
      Identifying <|-- UID
        click UID href "../UID/"
      Identifying <|-- Username
        click Username href "../Username/"
      Identifying <|-- VehicleLicense
        click VehicleLicense href "../VehicleLicense/"
      

      
```





## Inheritance
* [External](External.md)
    * **Identifying**
        * [Name](Name.md)
        * [OfficialID](OfficialID.md)
        * [Picture](Picture.md)
        * [UID](UID.md)
        * [Username](Username.md)
        * [VehicleLicense](VehicleLicense.md) [ [Vehicle](Vehicle.md)]


## Class Properties

| Property | Value |
| --- | --- |
| Class URI | [pd:Identifying](https://w3id.org/lmodel/dpv/pd/Identifying) |


## Slots

| Name | Cardinality and Range | Description | Inheritance |
| ---  | --- | --- | --- |











## In Subsets


* [PdSubset](PdSubset.md)



## Aliases


* Identifying




## Identifier and Mapping Information



### Annotations

| property | value |
| --- | --- |
| upstream_iri | https://w3id.org/dpv/pd/owl#Identifying |
| dpv_extension_slug | pd |




### Schema Source


* from schema: https://w3id.org/lmodel/dpv/pd




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | pd:Identifying |
| native | pd:Identifying |
| exact | dpv_pd:Identifying, dpv_pd_owl:Identifying |
| close | iso29100:PersonallyIdentifiableInformation |






## LinkML Source

<!-- TODO: investigate https://stackoverflow.com/questions/37606292/how-to-create-tabbed-code-blocks-in-mkdocs-or-sphinx -->

### Direct

<details>
```yaml
name: Identifying
annotations:
  upstream_iri:
    tag: upstream_iri
    value: https://w3id.org/dpv/pd/owl#Identifying
  dpv_extension_slug:
    tag: dpv_extension_slug
    value: pd
description: 'Information that uniquely or semi-uniquely identifies an individual
  or a

  group'
in_subset:
- pd_subset
from_schema: https://w3id.org/lmodel/dpv/pd
aliases:
- Identifying
exact_mappings:
- dpv_pd:Identifying
- dpv_pd_owl:Identifying
close_mappings:
- iso29100:PersonallyIdentifiableInformation
is_a: External
class_uri: pd:Identifying

```
</details>

### Induced

<details>
```yaml
name: Identifying
annotations:
  upstream_iri:
    tag: upstream_iri
    value: https://w3id.org/dpv/pd/owl#Identifying
  dpv_extension_slug:
    tag: dpv_extension_slug
    value: pd
description: 'Information that uniquely or semi-uniquely identifies an individual
  or a

  group'
in_subset:
- pd_subset
from_schema: https://w3id.org/lmodel/dpv/pd
aliases:
- Identifying
exact_mappings:
- dpv_pd:Identifying
- dpv_pd_owl:Identifying
close_mappings:
- iso29100:PersonallyIdentifiableInformation
is_a: External
class_uri: pd:Identifying

```
</details></div>