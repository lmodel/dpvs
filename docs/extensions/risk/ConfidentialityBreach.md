---
search:
  boost: 10.0
---

# Class: ConfidentialityBreach 


_Concept representing a breach of confidentiality_



<div data-search-exclude markdown="1">



URI: [risk:ConfidentialityBreach](https://w3id.org/lmodel/dpv/risk/ConfidentialityBreach)





```mermaid
 classDiagram
    class ConfidentialityBreach
    click ConfidentialityBreach href "../ConfidentialityBreach/"
      ConfidentialityConcept <|-- ConfidentialityBreach
        click ConfidentialityConcept href "../ConfidentialityConcept/"
      PotentialConsequence <|-- ConfidentialityBreach
        click PotentialConsequence href "../PotentialConsequence/"
      PotentialRisk <|-- ConfidentialityBreach
        click PotentialRisk href "../PotentialRisk/"
      PotentialRiskSource <|-- ConfidentialityBreach
        click PotentialRiskSource href "../PotentialRiskSource/"
      DataBreach <|-- ConfidentialityBreach
        click DataBreach href "../DataBreach/"
      
      
```





## Inheritance
* [TechnicalRiskConcept](TechnicalRiskConcept.md) [ [PotentialConsequence](PotentialConsequence.md) [PotentialImpact](PotentialImpact.md) [PotentialRisk](PotentialRisk.md) [PotentialRiskSource](PotentialRiskSource.md)]
    * [OperationalSecurityRisk](OperationalSecurityRisk.md) [ [PotentialConsequence](PotentialConsequence.md) [PotentialRisk](PotentialRisk.md) [PotentialRiskSource](PotentialRiskSource.md)]
        * [SecurityBreach](SecurityBreach.md) [ [AvailabilityConcept](AvailabilityConcept.md) [ConfidentialityConcept](ConfidentialityConcept.md) [IntegrityConcept](IntegrityConcept.md) [PotentialConsequence](PotentialConsequence.md) [PotentialRisk](PotentialRisk.md) [PotentialRiskSource](PotentialRiskSource.md)]
            * [DataBreach](DataBreach.md) [ [AvailabilityConcept](AvailabilityConcept.md) [ConfidentialityConcept](ConfidentialityConcept.md) [IntegrityConcept](IntegrityConcept.md) [PotentialConsequence](PotentialConsequence.md) [PotentialRisk](PotentialRisk.md) [PotentialRiskSource](PotentialRiskSource.md)]
                * **ConfidentialityBreach** [ [ConfidentialityConcept](ConfidentialityConcept.md) [PotentialConsequence](PotentialConsequence.md) [PotentialRisk](PotentialRisk.md) [PotentialRiskSource](PotentialRiskSource.md)]


## Class Properties

| Property | Value |
| --- | --- |
| Class URI | [risk:ConfidentialityBreach](https://w3id.org/lmodel/dpv/risk/ConfidentialityBreach) |


## Slots

| Name | Cardinality and Range | Description | Inheritance |
| ---  | --- | --- | --- |











## In Subsets


* [RiskSubset](RiskSubset.md)



## Aliases


* Confidentiality Breach




## Identifier and Mapping Information



### Annotations

| property | value |
| --- | --- |
| upstream_iri | https://w3id.org/dpv/risk/owl#ConfidentialityBreach |
| dpv_extension_slug | risk |




### Schema Source


* from schema: https://w3id.org/lmodel/dpv/risk




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | risk:ConfidentialityBreach |
| native | risk:ConfidentialityBreach |
| exact | dpv_risk:ConfidentialityBreach, dpv_risk_owl:ConfidentialityBreach |






## LinkML Source

<!-- TODO: investigate https://stackoverflow.com/questions/37606292/how-to-create-tabbed-code-blocks-in-mkdocs-or-sphinx -->

### Direct

<details>
```yaml
name: ConfidentialityBreach
annotations:
  upstream_iri:
    tag: upstream_iri
    value: https://w3id.org/dpv/risk/owl#ConfidentialityBreach
  dpv_extension_slug:
    tag: dpv_extension_slug
    value: risk
description: Concept representing a breach of confidentiality
in_subset:
- risk_subset
from_schema: https://w3id.org/lmodel/dpv/risk
aliases:
- Confidentiality Breach
exact_mappings:
- dpv_risk:ConfidentialityBreach
- dpv_risk_owl:ConfidentialityBreach
is_a: DataBreach
mixins:
- ConfidentialityConcept
- PotentialConsequence
- PotentialRisk
- PotentialRiskSource
class_uri: risk:ConfidentialityBreach

```
</details>

### Induced

<details>
```yaml
name: ConfidentialityBreach
annotations:
  upstream_iri:
    tag: upstream_iri
    value: https://w3id.org/dpv/risk/owl#ConfidentialityBreach
  dpv_extension_slug:
    tag: dpv_extension_slug
    value: risk
description: Concept representing a breach of confidentiality
in_subset:
- risk_subset
from_schema: https://w3id.org/lmodel/dpv/risk
aliases:
- Confidentiality Breach
exact_mappings:
- dpv_risk:ConfidentialityBreach
- dpv_risk_owl:ConfidentialityBreach
is_a: DataBreach
mixins:
- ConfidentialityConcept
- PotentialConsequence
- PotentialRisk
- PotentialRiskSource
class_uri: risk:ConfidentialityBreach

```
</details></div>